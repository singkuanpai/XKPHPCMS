...
function AttachReset() {
	if(bEditMode){
		editor.document.body.innerHTML = ContentLoad.value;
	}else{
...
function GetClipboardHTML() {
	var oDiv = document.getElementById("editor_Temp_HTML")
	oDiv.innerHTML = "" ;
...
function setHTML(html) {
	...
	case "CODE":
		...
		editor.document.write(getStyleEditorHeader());
	...
	case "EDIT":
		editor.document.designMode="On";
		editor.document.open();
		editor.document.write(getStyleEditorHeader()+html);
		editor.document.body.contentEditable="true";
		...
		bEditMode=true;
		editor.document.onselectionchange = function () { doToolbar();}
		break;
	case "TEXT":
		editor.document.designMode="On";
		editor.document.open();
	...
	editor.document.oncontextmenu=new Function("return showContextMenu(editor.event);");
...
function getHTML() {
	...
	if((sCurrMode=="EDIT")||(sCurrMode=="VIEW")){
		...
		html = editor.document.body.innerText;
	}
	if (sCurrMode!="TEXT"){
...
function setMode(NewMode){
	if (NewMode!=sCurrMode){
		...
		case "CODE":
			if (NewMode=="TEXT"){
				editor_Temp_HTML.innerHTML = editor.document.body.innerText;
				sBody = editor_Temp_HTML.innerText;
			}else{
				sBody = editor.document.body.innerText;
			}
			break;
		case "TEXT":
			sBody = editor.document.body.innerText;
		...
		case "VIEW":
			if (NewMode=="TEXT"){
				...
				sBody = editor.document.body.innerHTML;
			}
			break;
		default:
		...
		try{
			document.all["editor_CODE"].className = "StatusBarBtnOff";
			document.all["editor_EDIT"].className = "StatusBarBtnOff";
			document.all["editor_TEXT"].className = "StatusBarBtnOff";
			document.all["editor_VIEW"].className = "StatusBarBtnOff";
			document.all["editor_"+NewMode].className = "StatusBarBtnOn";
			}
		catch(e){
			}
		...
		setHTML(sBody);
		disableChildren(editor_Toolbar);
...
function findParentElement(tag) {
	var el = null;
	if (editor.document.selection.type != "Control") {
		el = editor.document.selection.createRange().parentElement();
...
function tagInSelection(tag){
	...
	if (sel.type!="Control"){
		var oBody = editor.document.body;
