...
require_once '../../admin/inc_checkadmin.php';
require("private.php");



$sPosition = $sPosition."样式管理";
$sAction == "STYLEPREVIEW";
...
function ShowStyleForm($sFlag){
	...
	if ($sFlag == "ADD"){
		...
		$GLOBALS["sStyleBaseHref"] = "http://Localhost/editor/";
	...
	echo "<table border=0 cellpadding=0 cellspacing=1 align=center class=form>".
			...
			"<tr><td><input type=hidden class=input size=20 name=d_name value=\"".$GLOBALS["sStyleName"]."\">".
			" ".$s_FormInitMode." ".
			" ".$s_FormUploadObject." ".
			" ".$s_FormAutoDir." ".
			"<input type=text class=input size=20 name=d_dir value=\"".$GLOBALS["sStyleDir"]."\">".
			"<input type=text class=input size=20 name=d_css value=\"".$GLOBALS["sStyleCSS"]."\">".
			"<input type=text class=input name=d_width size=20 value='".$GLOBALS["sStyleWidth"]."'>".
			"<input type=text class=input name=d_height size=20 value='".$GLOBALS["sStyleHeight"]."'>".
			" ".$s_FormStateFlag."  ".$s_FormDetectFromWord."  ".
			" ".$s_FormAutoRemote."  ".$s_FormShowBorder."  ".
			" ".$s_FormAutoDetectLanguage."  ".$s_FormDefaultLanguage."  ".
		  " ".$s_FormAllowBrowse."  ".
			"<input type=text name=d_memo size=90 value=\"".$GLOBALS["sStyleMemo"]."\">".
			" ".$s_FormBaseUrl."<input type=text class=input size=20 name=d_uploaddir value=\"".$GLOBALS["sStyleUploadDir"]."\">".
...
function ShowStylePreview(){
	echo "<html><head>".
		...
		"<iframe ID='editor1' src='../editor.htm?id=content1&style=".$GLOBALS["sStyleName"]."' frameborder=0 scrolling=no width='".$GLOBALS["sStyleWidth"]."' HEIGHT='".$GLOBALS["sStyleHeight"]."'></iframe>".
		"</body></html>";
}
...
function ShowStyleCode(){
	echo "<table border=0 cellspacing=1 align=center class=list>".
		...
		"<tr><td><textarea rows=5 cols=65 style='width:100%'><IFRAME ID=\"editor1\" SRC=\"editor.htm?id=XXX&style=".$GLOBALS["sStyleName"]."\" FRAMEBORDER=\"0\" SCROLLING=\"no\" WIDTH=\"".$GLOBALS["sStyleWidth"]."\" HEIGHT=\"".$GLOBALS["sStyleHeight"]."\"></IFRAME></textarea></td></tr>".
		"</table>";
}
...
function WriteStyle($n_StyleID){
	...
	$sConfig = $sConfig."<table border=0 cellpadding=0 cellspacing=0 width='100%' class='Toolbar' id='editor_Toolbar'>";
